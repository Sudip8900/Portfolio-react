    ...
    return (
        <>
            <div ref={navref2} className="hidden md:flex fixed top-4 left-4 right-4 z-50 items-center justify-center gap-4 pointer-events-none">
                ...
                {/* Main Glass Navbar */}
                <nav
                    className="flex items-center justify-between px-10 py-5 rounded-full bg-black/60 backdrop-blur-[32px] backdrop-saturate-[180%] border border-white/10 w-full max-w-7xl relative pointer-events-auto shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
                    {/* Logo */}
                    <div className="text-2xl font-bold tracking-widest uppercase text-white flex items-center gap-2">
                        ...
                        {sectionsList.map((section, index) => (
                            <Magnetic key={index}>
                                <div className="inline-block relative py-1" ref={(el) => (linkRef2.current[index] = el)}>
                                    <Link
                                        spy={true}
                                        activeClass='text-white pointer-events-none scale-110 drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]'
                                        className="transition-all duration-300 cursor-pointer text-white/50 hover:text-white uppercase inline-block"
                                        to={section}
                                        smooth
                                        duration={200}
                                        offset={-100}
                ...
                <div className="relative group">
                    <button 
                        ...
                        className="flex items-center justify-center p-5 rounded-full bg-black/60 backdrop-blur-[32px] backdrop-saturate-[180%] border border-white/10 text-white pointer-events-auto shadow-[0_8px_32px_rgba(0,0,0,0.5)] hover:text-orange-500 hover:border-orange-500 transition-all duration-300 z-50"
                    >
            ...
            <nav ref={navRef} className='fixed z-50 w-full h-full flex flex-col justify-between px-10 uppercase bg-black/90 backdrop-blur-xl border-l border-white/10 text-white/80 py-32 gap-y-10 md:hidden rounded-l-[40px] shadow-2xl'>
            ...
            {/* Mobile Top Bar */}
            <div>
                <div ref={titleRef} className='text-xl font-bold tracking-widest uppercase text-white fixed m-5 md:hidden bg-black/40 backdrop-blur-md border border-white/10 shadow-lg px-6 py-3 rounded-full z-40 flex items-center gap-2'>
                    <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></span>
                    <h1>SUDIP.DEV</h1>
                </div>
                <div className="fixed z-[60] top-5 right-24 md:hidden pointer-events-none">
                    <button onClick={() => { toggleTheme(); if(showHint) dismissHint(); }} className='flex items-center justify-center text-white bg-black/40 backdrop-blur-md border border-white/10 rounded-full cursor-pointer w-14 h-14 shadow-lg hover:border-orange-500 hover:text-orange-500 transition-colors pointer-events-auto'>
                        {theme === 'light' ? <Icon icon="solar:moon-bold" width="24" height="24" /> : <Icon icon="solar:sun-bold" width="24" height="24" />}
                    </button>
                    {showHint && (
                        <div className="absolute top-full right-0 mt-4 w-40 bg-black/80 backdrop-blur-md border border-orange-500/50 rounded-xl p-3 shadow-[0_5px_20px_rgba(255,105,0,0.2)] pointer-events-auto animate-bounce z-50">
                ...
                <div ref={BurgerRef} className='fixed z-[60] flex flex-col items-center justify-center gap-1.5 transition-all duration-300 bg-black/40 backdrop-blur-md border border-white/10 rounded-full cursor-pointer w-14 h-14 top-5 right-5 md:hidden shadow-lg hover:border-orange-500' onClick={toggleMenue}>
